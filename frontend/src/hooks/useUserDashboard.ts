...
export function useUserYesNoDistribution(address?: `0x${string}`) {
    const { bets, loading } = useUserDashboardData(address);
    const [distribution, setDistribution] = useState<BetDistribution>({
        yes: 0,
        no: 0,
    });
...
export function useUserWinsLosses(address?: `0x${string}`, days: number = 7) {
    ...
    useEffect(() => {
        ...
        for (let i = days - 1; i >= 0; i--) {
            const timestamp = now - i * 24 * 60 * 60 * 1000;
            const date = new Date(timestamp);
            const dayStart = new Date(timestamp).setHours(0, 0, 0, 0) / 1000;
...
export function useUserWinsLossesByMarket(
    address?: `0x${string}`,
    days: number = 7
) {
    const { bets, loading } = useUserDashboardData(address);
    ...
    useEffect(() => {
        ...
        for (const bet of bets) {
            ...
            for (let i = 0; i < dataPoints.length; i++) {
                ...
                const dayStart =
                    new Date(
                        new Date(dp.timestamp).setHours(0, 0, 0, 0)
                    ).getTime() / 1000;
                const dayEnd = dayStart + 86400;

                if (
                    resolutionTimestampSec >= dayStart &&
                    resolutionTimestampSec < dayEnd
                ) {
                    if (isWin) {
                        dp.wins++;
                    } else {
